...
import { isRole, Role } from './authz/roles';
...
const preferredFrontendDirectory = fs.existsSync(
  path.join(runtimeDistPublicDirectory, 'index.html'),
)
...
const staticAssetDirectories = [
  ...
  ...(preferredFrontendDirectory === workspaceSourcePublicDirectory
    ? []
    : [workspaceSourcePublicDirectory]),
];
...
function getParticipantIdFromBody(req) {
  ...
  if (
    typeof req.body?.previousParticipantId === 'string' &&
...
function signOwnedRoomsToken(ownedRoomIds) {
  return jwt.sign({ ownedRoomIds }, config.auth.jwtSecret, {
...
function createAccessSession(role, roomId = null) {
  ...
  const expiresAt = new Date(
    issuedAt.getTime() + AUTH_TOKEN_TTL_SECONDS * 1000,
  );
...
function getInviteSessionFromRequest(req) {
  ...
  const requestedRoomId =
    typeof req.query.room === 'string' &&
      ROOM_ID_PATTERN.test(req.query.room)
      ? req.query.room
      : null;
  ...
  try {
    ...
    logger.warn(
      { err: error, roomId: requestedRoomId },
      'Ignoring invalid invite token',
    );
    return null;
  }
}
...
function getBootstrapSessionFromRequest(req) {
  ...
  try {
    ...
    return {
      ...
      roomId:
        typeof parsed.roomId === 'string' &&
          ROOM_ID_PATTERN.test(parsed.roomId)
          ? parsed.roomId
          : null,
    };
  } catch (error) {
    logger.warn(
      { err: error },
      'Ignoring invalid bootstrap session cookie',
    );
    return null;
  }
}
...
function tryRestoreOwnerAuthentication(req, res) {
  ...
  logger.info(
    { path: req.path, roomId: requestedRoomId },
    'Restored host authentication from owner cookie',
  );
  return true;
}
...
function tryRestoreLinkAuthentication(req, res) {
  ...
  logger.info(
    { path: req.path, roomId: requestedRoomId },
    'Restored guest authentication from room link',
  );
  return true;
}
...
function getBootstrapDebugMetadata(req) {
  return {
    ...
    roomIdQuery:
      typeof req.query?.roomId === 'string' ? req.query.roomId : null,
    sessionIdQuery:
  ...
  logger.info(
    {
      ...
      bootstrapRoomId:
        getBootstrapSessionFromRequest(req)?.roomId ?? null,
  ...
  try {
    ...
    if (refreshToken) {
      try {
        ...
        logger.warn(
          { err: refreshError, path: req.path },
          'Invalid refresh token after access token failure',
        );
      }
    }
  ...
  return async (req, res, next) => {
    try {
      ...
      if (req.auth?.roomId) {
        ...
        if (participantId) {
          const storedRole = storedSession?.getParticipantRole?.(
      ...
      const authorization = canPerform(
        { role: effectiveRole },
        permission,
      );
      ...
      logger.error(
        { err: error, path: req.path, permission },
        'Failed to resolve effective role for permission check',
      );
...
function getParticipantIdFromRequest(req) {
  ...
  if (
    typeof req.query.participantId === 'string' &&
...
async function getAuthorizationContextForRequest(req) {
  ...
  if (req.auth?.roomId) {
    ...
    if (participantId) {
      grants = storedSession.getParticipantPermissions(
        new ParticipantId(participantId),
      );
    }
  }
...
async function loadStoredSessionOrFail(sessionId) {
  ...
  throw createHttpError(
    404,
    `Session ${sessionId} is not registered in the repository`,
  );
}
...
function resolveRootParticipantId(
  storedSession,
  participantIdOrConnectionId = null,
) {
  ...
  if (
    storedSession.hasParticipant(
      new ParticipantId(participantIdOrConnectionId),
    )
  ) {
  ...
  return (
    storedSession.findParticipantIdByMediaConnection(
      participantIdOrConnectionId,
    ) || null
  );
}
...
function getRoomTargetForParticipant(storedSession, participantId = null) {
  ...
  const location = storedSession.getParticipantLocation(
    new ParticipantId(participantId),
  );
...
function buildBreakoutSnapshot(storedSession) {
  return {
    ...
    participantMediaConnections:
      storedSession.getParticipantMediaConnections(),
...
function summarizeStoredSessionForLogs(storedSession) {
  ...
  return {
    revision: storedSession.getRevision(),
    connectedParticipantCount: Object.values(participantPresence).filter(
      (presence) => presence === 'connected',
    ).length,
    waitingRoomRequestCount:
      storedSession.getPendingWaitingRoomRequests().length,
    waitingRoomRequests: storedSession
      .getPendingWaitingRoomRequests()
      .map((request) => ({
        id: request.id,
...
function resolveWhiteboardLocation(payload = {}) {
  if (payload?.roomType === 'breakout') {
    if (
      ...
      !isValidBreakoutRoomId(payload.breakoutRoomId)
    ) {
      throw new Error(
        'A valid breakoutRoomId is required for breakout whiteboard state',
      );
    }
...
function broadcastRoomTransferRequested(
  rootSessionId,
  storedSession,
  participantId,
  targetRoom,
...
function canManageRequestedPermissions(role, permissionPatch) {
  ...
  const needsWhiteboardManagement =
    permissionPatch.whiteboardEnabled !== undefined;
...
function requiresWaitingRoomApproval(
  role,
  participantId = null,
...
app.get(
  ...
  requirePermission(Permission.JOIN_SESSION),
  (req, res) => {
    ...
    logger.info(
      ...
      'Providing Azure Speech credentials',
    );
    res.json({
  ...
  async (req, res) => {
    try {
      ...
      const authorizationContext =
        await getAuthorizationContextForRequest(
          Object.assign(
      ...
      res.json({
        ...
        breakoutRooms: storedSession?.getBreakoutRooms?.() || [],
        waitingRoomRequests:
          storedSession?.getPendingWaitingRoomRequests?.() || [],
        participantLocations:
          storedSession?.getParticipantLocations?.() || {},
        participantProfiles:
          storedSession?.getParticipantProfiles?.() || {},
        participantRoles: storedSession?.getParticipantRoles?.() || {},
        participantPermissions:
          storedSession?.getAllParticipantPermissions?.() || {},
        participantPresence:
          storedSession?.getParticipantPresenceMap?.() || {},
        participantMediaConnections:
          storedSession?.getParticipantMediaConnections?.() || {},
        whiteboardState:
          storedSession?.getWhiteboardStateSnapshot?.() || {
            main: {
              isOpen: false,
              canvasState: null,
              updatedAt: null,
      ...
      logger.error(
        { err: error },
        'Failed to resolve current permissions',
      );
      res.status(500).json({
...
app.get(
  '/api/sessions/:sessionId/invite-link',
...
app.get(
  '/api/sessions/:sessionId/room-target',
  requirePermission(Permission.JOIN_SESSION),
  async (req, res) => {
    try {
      const { sessionId } = req.params;
      ...
      const authorizationContext =
        await getAuthorizationContextForRequest(req);
      const roomTarget = getRoomTargetForParticipant(
        storedSession,
        participantId,
      );
...
app.get(
  '/api/sessions/:sessionId/breakouts',
  requirePermission(Permission.JOIN_SESSION),
  async (req, res) => {
    try {
      const { sessionId } = req.params;
...
app.post(
  '/api/sessions/:sessionId/waiting-room/requests',
  requirePermission(Permission.JOIN_SESSION),
  async (req, res) => {
    try {
      const { sessionId } = req.params;
      ...
      if (!nickname) {
        return res.status(400).json({
          ...
          details:
            'A nickname is required to create a waiting room request',
        });
      }
      ...
      if (
        participantId ||
        req.auth.role === Role.HOST ||
        req.auth.role === Role.CO_HOST
      ) {
        return res.json({
      ...
      const existingRequest =
        storedSession.getWaitingRoomRequest(requestId);
      const requestedAt =
      ...
      storedSession.upsertWaitingRoomRequest({
        ...
        role: req.auth.role,
        status:
          existingRequest?.status || WaitingRoomRequestStatus.PENDING,
        requestedAt,
      ...
      logger.info(
        ...
        existingRequest
          ? 'Updated waiting room request'
          : 'Created waiting room request',
      );
      ...
      logger.error(
        { err: error },
        'Failed to create waiting room request',
      );
...
app.get(
  '/api/sessions/:sessionId/waiting-room/requests/:requestId',
  requirePermission(Permission.JOIN_SESSION),
  async (req, res) => {
    try {
      ...
      logger.error(
        { err: error },
        'Failed to fetch waiting room request',
      );
      return res.status(error.status || 500).json({
...
app.delete(
  ...
  async (req, res) => {
    try {
      ...
      logger.error(
        { err: error },
        'Failed to remove waiting room request',
      );
      return res.status(error.status || 500).json({
...
app.post(
  '/api/sessions/:sessionId/waiting-room/requests/:requestId/approve',
  requirePermission(Permission.ADMIT_WAITING_ROOM),
  async (req, res) => {
    try {
      const { sessionId, requestId } = req.params;
      if (!validateRoomBinding(req, res, sessionId)) {
      ...
      if (request.status !== WaitingRoomRequestStatus.APPROVED) {
        storedSession.approveWaitingRoomRequest(
          requestId,
      ...
      logger.error(
        { err: error },
        'Failed to approve waiting room request',
      );
      return res.status(error.status || 500).json({
...
app.post(
  '/api/sessions/:sessionId/waiting-room/requests/:requestId/reject',
  requirePermission(Permission.ADMIT_WAITING_ROOM),
  async (req, res) => {
    try {
      const { sessionId, requestId } = req.params;
      ...
      if (request.status !== WaitingRoomRequestStatus.REJECTED) {
        storedSession.rejectWaitingRoomRequest(
          requestId,
          new Date().toISOString(),
          req.auth.role,
        );
      ...
      logger.error(
        { err: error },
        'Failed to reject waiting room request',
      );
      return res.status(error.status || 500).json({
...
app.post(
  '/api/sessions/:sessionId/breakouts',
  requirePermission(Permission.MANAGE_BREAKOUT_ROOMS),
  async (req, res) => {
    try {
      const { sessionId } = req.params;
      const requestedName =
      ...
      const breakoutRoomName =
        requestedName ||
        `Breakout ${Date.now().toString(36).slice(-4).toUpperCase()}`;
...
app.patch(
  '/api/sessions/:sessionId/breakouts/:breakoutRoomId',
  requirePermission(Permission.MANAGE_BREAKOUT_ROOMS),
  async (req, res) => {
    try {
...
app.post(
  '/api/sessions/:sessionId/breakouts/:breakoutRoomId/open',
  requirePermission(Permission.MANAGE_BREAKOUT_ROOMS),
  async (req, res) => {
    try {
      const { sessionId, breakoutRoomId } = req.params;
      ...
      const livekitRoomName =
        breakoutRoom.livekitRoomName ||
        getBreakoutLivekitRoomName(sessionId, breakoutRoomId);
...
app.post(
  '/api/sessions/:sessionId/breakouts/:breakoutRoomId/close',
  requirePermission(Permission.MANAGE_BREAKOUT_ROOMS),
  async (req, res) => {
    try {
      const { sessionId, breakoutRoomId } = req.params;
      ...
      storedSession.closeBreakoutRoom(
        breakoutRoomId,
        new Date().toISOString(),
      );
      await sessionRepository.save(storedSession);
  ...
  async (req, res) => {
    try {
      ...
      const connectedParticipantIds = (
        breakoutRoom.participantIds || []
      ).filter((participantId) => {
        return (
          storedSession.getParticipantPresence(
            new ParticipantId(participantId),
          ) === 'connected'
        );
      });
      ...
      if (connectedParticipantIds.length > 0) {
        return res.status(409).json({
          ...
          details:
            'Connected participants must return to the main room before deleting this breakout room',
        });
      }
...
app.put(
  ...
  requirePermission(Permission.MOVE_PARTICIPANT_BETWEEN_ROOMS),
  async (req, res) => {
    try {
      ...
      const resolvedParticipantId = resolveRootParticipantId(
        storedSession,
        participantId,
      );
      ...
      const targetType =
        req.body?.target === 'breakout' ? 'breakout' : 'main';
      const breakoutRoomId = req.body?.breakoutRoomId;
      const participantPresence =
        storedSession.getParticipantPresence(participantRef);
      ...
      if (targetType === 'breakout') {
        ...
        const breakoutRoom =
          storedSession.getBreakoutRoom(breakoutRoomId);
        if (!breakoutRoom) {
          return res.status(404).json({
            error: 'Breakout room not found',
            details: `Breakout room ${breakoutRoomId} does not exist`,
        ...
        storedSession.assignParticipantToBreakout(
          participantRef,
          breakoutRoomId,
        );
      } else {
      ...
      logger.info(
        {
          ...
          targetType,
          breakoutRoomId:
            targetType === 'breakout' ? breakoutRoomId : null,
          resultingRoomTarget: getRoomTargetForParticipant(
            storedSession,
            resolvedParticipantId,
          ),
          snapshot: summarizeStoredSessionForLogs(storedSession),
      ...
      return res.json({
        ...
        roomTarget: getRoomTargetForParticipant(
          storedSession,
          resolvedParticipantId,
        ),
      });
      ...
      logger.error(
        { err: error },
        'Failed to move participant between rooms',
      );
      return res.status(error?.status || 400).json({
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const isSelfDisconnect =
        requestParticipantId &&
        resolvedParticipantId &&
        requestParticipantId === resolvedParticipantId;
      const canModerateDisconnect =
        canPerform({ role: req.auth.role }, Permission.KICK_PARTICIPANT)
          .allowed ||
        canPerform(
          { role: req.auth.role },
      ...
      const disconnectingRole =
        storedSession.getParticipantRole(participantRef);
      ...
      if (disconnectingRole === Role.HOST) {
        ...
        const connectedCoHost = Object.entries(roles).find(
          ([pid, role]) =>
            role === Role.CO_HOST && presence[pid] === 'connected',
        );

        // Promote co-host and demote the disconnecting host so there's never two hosts
        if (connectedCoHost) {
          storedSession.setParticipantRole(
            new ParticipantId(connectedCoHost[0]),
            Role.HOST,
          );
          storedSession.setParticipantRole(
            new ParticipantId(resolvedParticipantId),
            Role.CO_HOST,
          );
        } else {
          ...
          for (const [pid, status] of Object.entries(presence)) {
            if (
              status === 'connected' &&
              roles[pid] === Role.PARTICIPANT
            ) {
              storedSession.updateParticipantPermissions(
                new ParticipantId(pid),
                {
                  audioEnabled: false,
                  videoEnabled: false,
                },
          ...
          logger.info(
            { sessionId },
            'Revoked audio/video for all participants — no co-host available',
          );
        }
        await sessionRepository.save(storedSession);
        ...
        broadcastPermissionUpdate(sessionId, {
          ...
          promotedParticipantId: connectedCoHost
            ? connectedCoHost[0]
            : null,
          ...buildBreakoutSnapshot(storedSession),
        });
        logger.info(
          {
            sessionId,
            hasCoHost: Boolean(connectedCoHost),
            promotedCoHost: connectedCoHost?.[0] || null,
          },
      ...
      logger.error(
        { err: error },
        'Failed to mark participant disconnected',
      );
      return res.status(error?.status || 400).json({
...
app.post(
  '/api/sessions/:sessionId/end-meeting',
  requirePermission(Permission.END_SESSION),
  async (req, res) => {
    try {
      const { sessionId } = req.params;
      if (!validateRoomBinding(req, res, sessionId)) {
          ...
          if (mediaId) {
            removalPromises.push(
              removeLivekitParticipant(
                ...
                logger.warn(
                  { err, participantId: pid },
                  'Failed to remove participant from LiveKit during end-meeting',
                ),
              ),
            );
          }
          storedSession.markParticipantDisconnected(
            new ParticipantId(pid),
          );
        }
      }
...
app.post(
  '/api/sessions/:sessionId/transfer-host',
  requirePermission(Permission.END_SESSION),
  async (req, res) => {
    try {
      const { sessionId } = req.params;
      ...
      if (!targetParticipantId) {
        return res.status(400).json({
          ...
          details: 'You must specify the co-host to promote.',
        });
      }

      const storedSession = await getConsistentStoredSession(sessionId);
      const resolvedTargetId = resolveRootParticipantId(
        storedSession,
        targetParticipantId,
      );
      if (!resolvedTargetId) {
        return res.status(404).json({
          error: 'Participant not found',
          details: `Participant ${targetParticipantId} not found.`,
        });
      }
      ...
      if (targetRole !== Role.CO_HOST) {
        return res.status(400).json({
          ...
          details: 'Host can only be transferred to a co-host.',
        });
      }

      const targetPresence =
        storedSession.getParticipantPresence(targetRef);
      if (targetPresence !== 'connected') {
        return res.status(400).json({
          error: 'Target not connected',
          details: 'The target co-host is not currently connected.',
        });
      }
      ...
      const resolvedCurrentHostId = resolveRootParticipantId(
        storedSession,
        currentHostId,
      );
      if (resolvedCurrentHostId) {
        storedSession.setParticipantRole(
          new ParticipantId(resolvedCurrentHostId),
          Role.CO_HOST,
        );
      }
      ...
      logger.info(
        {
          sessionId,
          newHost: resolvedTargetId,
          previousHost: resolvedCurrentHostId,
        },
    ...
    try {
      ...
      if (Object.keys(permissionPatch).length === 0) {
        return res.status(400).json({
          ...
          details:
            'At least one supported permission flag must be provided',
        });
      }

      const authorization = canManageRequestedPermissions(
        req.auth.role,
        permissionPatch,
      );
      if (!authorization.allowed) {
      ...
      const targetRole =
        storedSession.getParticipantRole(participantRef) ||
        Role.PARTICIPANT;
      const targetRoleAuthorization = canModerateTargetRole(
        req.auth.role,
        targetRole,
      );
      ...
      const participantPermissions =
        storedSession.getParticipantPermissions(participantRef);
      logger.info(
        {
          sessionId,
          resolvedParticipantId,
    ...
    try {
      ...
      if (currentRole === Role.HOST) {
        return res.status(409).json({
          ...
          details:
            'The host role cannot be changed from this endpoint',
        });
      }
...
app.patch(
  '/api/sessions/:sessionId/whiteboard-state',
  requirePermission(Permission.JOIN_SESSION),
  async (req, res) => {
    try {
      const { sessionId } = req.params;
      ...
      const requestedParticipantId = resolveRootParticipantId(
        storedSession,
        getParticipantIdFromRequest(req),
      );
      const authorizationContext =
      ...
      if (req.body?.canvasState !== undefined) {
        ...
        if (
          req.body.canvasState !== null &&
          typeof req.body.canvasState !== 'string'
        ) {
          return res.status(400).json({
...
app.delete(
  ...
  requirePermission(Permission.KICK_PARTICIPANT),
  async (req, res) => {
    try {
      const { sessionId, participantId } = req.params;
      ...
      const participantLocation =
        storedSession.getParticipantLocation(participantRef);
      const breakoutRoom =
        participantLocation.type === 'breakout' &&
          participantLocation.breakoutRoomId
          ? storedSession.getBreakoutRoom(
            participantLocation.breakoutRoomId,
          )
          : null;
      const targetRoomName = breakoutRoom?.livekitRoomName || sessionId;
      const mediaConnectionId =
        storedSession.getParticipantMediaConnection(participantRef);
      ...
      logger.error(
        { err: error },
        'Failed to remove participant from session',
      );
      return sendServiceError(
        res,
        error,
        'Failed to remove participant from the session',
      );
    }
  },
);
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      logger.info(
        {
          ...
          bootstrapRoomId:
            getBootstrapSessionFromRequest(req)?.roomId ?? null,
          bootstrapRole:
            getBootstrapSessionFromRequest(req)?.role ?? null,
        },
      ...
      const storedSession =
        await sessionRepository.findById(resolvedSessionId);
      ...
      const domainSession = DomainSession.create(
        resolvedSessionId,
  ...
  async (req, res) => {
    try {
      ...
      const participantReferenceId =
        (typeof rootParticipantId === 'string' &&
          rootParticipantId.trim()) ||
        (typeof previousParticipantId === 'string' &&
          previousParticipantId.trim()) ||
        null;
      const currentParticipantId = resolveRootParticipantId(
        storedSession,
      ...
      if (shouldRequireWaitingApproval) {
        if (!waitingRoomRequest) {
          ...
          return res.status(403).json({
            ...
            details:
              'A host or co-host must admit you before you can join this session',
            waitingRoomRequired: true,
            waitingRequestId: resolvedWaitingRequestId,
          });
        }
        ...
        if (waitingRoomRequest.role !== req.auth.role) {
          return res.status(403).json({
            ...
            details:
              'The waiting room request role does not match the authenticated role',
            waitingRoomRequired: true,
            waitingRequestId: resolvedWaitingRequestId,
          });
        }
        ...
        if (
          waitingRoomRequest.status ===
          WaitingRoomRequestStatus.REJECTED
        ) {
          return res.status(403).json({
        ...
        if (
          waitingRoomRequest.status !==
          WaitingRoomRequestStatus.APPROVED
        ) {
          return res.status(403).json({
            error: 'Waiting room approval required',
            details:
      ...
      const participantMetadata = {
        ...
        isScreenShare: Boolean(
          isAuxiliaryMedia && auxiliaryMediaKind === 'screen-share',
        ),
        auxiliaryMediaKind: isAuxiliaryMedia
          ? auxiliaryMediaKind || 'auxiliary'
          : null,
      };
      ...
      if (!isAuxiliaryMedia && currentParticipantId) {
        const previousMediaConnectionId =
          storedSession.getParticipantMediaConnection(
            new ParticipantId(currentParticipantId),
          );
        if (previousMediaConnectionId) {
          await removeLivekitParticipant(
            targetRoomName,
            previousMediaConnectionId,
          );
        }
      }
      ...
      if (isAuxiliaryMedia) {
        ...
        storedSession.setParticipantProfile(
          participantRef,
          participantProfile,
        );
        storedSession.setParticipantRole(participantRef, req.auth.role);
        storedSession.setParticipantMediaConnection(
          participantRef,
          mediaConnectionId,
        );
        if (
          roomTarget.type === 'breakout' &&
      ...
      if (!isAuxiliaryMedia && req.auth.role === Role.HOST) {
        ...
        for (const [pid, role] of Object.entries(allRoles)) {
          if (role === Role.HOST && pid !== joiningId) {
            ...
            logger.info(
              {
                sessionId,
                demotedParticipant: pid,
                presence: allPresence[pid],
                returningHost: joiningId,
              },
        ...
        for (const [pid, pRole] of Object.entries(restoreRoles)) {
          ...
          const perms =
            storedSession.getParticipantPermissions(
              new ParticipantId(pid),
            ) || {};
          if (
            perms.audioEnabled === false ||
            perms.videoEnabled === false
          ) {
            // Remove the override — revert to role-based defaults
            const cleaned = { ...perms };
            delete cleaned.audioEnabled;
            delete cleaned.videoEnabled;
            storedSession.setParticipantPermissions(
              new ParticipantId(pid),
              cleaned,
            );
      ...
      logger.info(
        {
          ...
          snapshot: !isAuxiliaryMedia
            ? summarizeStoredSessionForLogs(storedSession)
            : undefined,
        },
...
app.patch(
  ...
  requirePermission(Permission.UPDATE_ROOM_CONFIGURATION),
  async (req, res) => {
    try {
      const { sessionId } = req.params;
      ...
      if (typeof roomConfiguration.groupChatEnabled === 'boolean') {
        configurationPatch.groupChatEnabled =
          roomConfiguration.groupChatEnabled;
      }

      if (typeof roomConfiguration.subtitlesEnabled === 'boolean') {
        configurationPatch.subtitlesEnabled =
          roomConfiguration.subtitlesEnabled;
      }
      ...
      if (
        ...
        configurationPatch.aiInterpretationEnabled =
          roomConfiguration.aiInterpretationEnabled;
      }
      ...
      if (Object.keys(configurationPatch).length === 0) {
        return res.status(400).json({
          ...
          details:
            'At least one supported room configuration flag must be provided',
        });
      }
...
class InterpreterSession {
  ...
  connectToOpenAI() {
    if (
      !config.voiceAi.apiKey ||
      config.voiceAi.apiKey === 'your-azure-voiceai-api-key-here'
    ) {
      this.logger.error('AZURE_VOICEAI_API_KEY not configured');
      this.sendToClient({
        type: 'error',
        message: 'Azure Voice AI API key not configured on server',
      });
      return;
    }
  ...
  configureSession() {
    ...
    const prompt = generateInterpreterPrompt(
      this.sourceLanguage,
      this.targetLanguage,
    );
...
function setupInterpreterWebSocket() {
  ...
  logger.info(
    { path: '/ws/interpret' },
    'OpenAI Realtime proxy WebSocket enabled',
  );
  return wss;
}
...
function setupPermissionsWebSocket() {
  ...
  wss.on('connection', (ws, req) => {
    ...
    if (!token) {
      wsLogger.warn(
        { requestedRoomId, participantId },
        'Permissions WebSocket missing auth token',
      );
    ...
    try {
      ...
      if (
        ...
        wsLogger.warn(
          { requestedRoomId, participantId },
          'Permissions WebSocket rejected invalid token payload',
        );
      ...
      if (requestedRoomId && requestedRoomId !== roomId) {
        wsLogger.warn(
          { requestedRoomId, roomId, participantId },
          'Permissions WebSocket room mismatch',
        );
      ...
      wsLogger.info(
        { roomId, role: decoded.role, participantId },
        'Permissions WebSocket connected',
      );
      ws.send(
        JSON.stringify({ type: 'connected', roomId, participantId }),
      );
      ...
      getConsistentStoredSession(roomId, { broadcast: false })
        ...
        .catch((error) => {
          wsLogger.warn(
            { err: error, roomId },
            'Failed to send initial breakout snapshot',
          );
        });
      ...
      ws.on('close', () => {
        ...
        wsLogger.info(
          ...
          'Permissions WebSocket disconnected',
        );
        ...
        if (participantId) {
          ...
          const captureSnapshotAndSchedule = async () => {
            ...
            try {
              const snapSession =
                await sessionRepository.findById(roomId);
              if (snapSession) {
                const snapResolved = resolveRootParticipantId(
                  snapSession,
                  participantId,
                );
                if (snapResolved) {
                  snapshotMediaConnectionId =
                    snapSession.getParticipantMediaConnection(
                      new ParticipantId(snapResolved),
                    );
                }
              }
            } catch (_) { }
            ...
            setTimeout(async () => {
              try {
                ...
                const resolvedId = resolveRootParticipantId(
                  storedSession,
                  participantId,
                );
                if (!resolvedId) return;

                const participantRef = new ParticipantId(
                  resolvedId,
                );
                const role =
                  storedSession.getParticipantRole(
                    participantRef,
                  );
                if (role !== Role.HOST) return;

                // Check if the host reconnected by comparing media connection IDs
                const currentMediaConnectionId =
                  storedSession.getParticipantMediaConnection(
                    participantRef,
                  );
                const presence =
                ...
                if (presence === 'connected') {
                  ...
                  await sessionRepository.save(storedSession);
                  wsLogger.info(
                    { roomId, participantId: resolvedId },
                    'Marked host disconnected via WebSocket close fallback',
                  );
                ...
                if (
                  ...
                  currentPresence !== 'disconnected'
                )
                  return;

                // Check if any connected participant already has the host role
                const allRoles =
                  storedSession.getParticipantRoles();
                const allPresence =
                  storedSession.getParticipantPresenceMap();
                const hasActiveHost = Object.entries(
                  allRoles,
                ).some(
                  ([pid, r]) =>
                    r === Role.HOST &&
                    allPresence[pid] === 'connected',
                );
                if (hasActiveHost) return;

                // Find a connected co-host to promote
                const connectedCoHost = Object.entries(
                  allRoles,
                ).find(
                  ([pid, r]) =>
                    r === Role.CO_HOST &&
                ...
                if (connectedCoHost) {
                  ...
                  wsLogger.info(
                    {
                      roomId,
                      promotedCoHost: connectedCoHost[0],
                    },
                  ...
                  const wsAllPresence =
                    storedSession.getParticipantPresenceMap();
                  for (const [pid, status] of Object.entries(
                    wsAllPresence,
                  )) {
                    if (
                      status === 'connected' &&
                      wsAllRoles[pid] === Role.PARTICIPANT
                    ) {
                      storedSession.updateParticipantPermissions(
                        new ParticipantId(pid),
                        {
                ...
                wsLogger.warn(
                  { err: error, roomId, participantId },
                  'Error in host disconnect fallback check',
                );
              }
            }, 30_000);
          };
          captureSnapshotAndSchedule().catch((err) => {
            wsLogger.warn(
              { err, roomId, participantId },
              'Error in host disconnect snapshot capture',
            );
          });
        }
      });
      ...
      ws.on('error', (error) => {
        wsLogger.error(
          { err: error, roomId, participantId },
          'Permissions WebSocket error',
        );
      });
    } catch (error) {
      wsLogger.warn(
        { err: error, requestedRoomId, participantId },
        'Rejected permissions WebSocket connection',
      );
      ws.close(4003, 'Invalid authorization token');
    }
  });
...
app.get('*', (req, res) => {
  ...
  if (incomingToken) {
    try {
      ...
      logger.warn(
        { err: error },
        'Ignoring invalid bootstrap auth cookie',
      );
    }
  }
